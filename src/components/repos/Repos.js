...
function Repos({ params }) {
  ...
  function filterRepos(filter = 'Stars') {
    const filteredValues = allRepos.map((repo) => {
      if (filter === 'Stars') return repo.stargazers_count;

      if (filter === 'Forks') return repo.forks;

      if (filter === 'Size') return repo.size;
    });
    ...
    <>
      <ReposStyled>
        <div className='order-line'>
          ...
          <p>by</p>
          <div className='list'>
            <button onClick={handleShowOptions} className='select-filter' value={filter}>
              {filter || 'Stars'}
              <ArrowDropDownIcon />
            </button>
            <ul className='list-options' ref={listOptRef}>
              <li className='option-filter'>
                <button className='btn-filter' value='Stars' onClick={selectOption}>
                  Stars
                </button>
              </li>
              <li className='option-filter'>
                <button className='btn-filter' value='Forks' onClick={selectOption}>
                  Forks
                </button>
              </li>
              <li className='option-filter'>
                <button className='btn-filter' value='Size' onClick={selectOption}>
                  Size
                </button>
              </li>
            </ul>
        ...
        <ListRepos reposFiltered={reposFiltered} allRepos={allRepos} filterText={filter} />
      </ReposStyled>
      <br />
      <br />
      <br />
...
const ReposStyled = styled.div`
  ...
  h2 {
    ...
    font-size: 2.2rem;
    font-weight: 500;
    border-bottom: 3px dashed rgba(7, 7, 7, 0.2);
  }
  p {
    font-family: 'Inter';
    font-size: 1.6rem;
  ...
  .btn-filter {
    ...
    border: 1px solid rgba(124, 127, 198, 0.4);
    border-radius: 10px;
    position: relative;
    font-size: 0.8rem;
    color: rgb(0, 112, 243);
